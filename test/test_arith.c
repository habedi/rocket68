...
void test_negx() {
    ...
    m68k_step(&cpu);
    assert(cpu.d_regs[0].l == 0xFFFFFFFF);
...
void test_bcd() {
    ...
    assert((cpu.d_regs[1].l & 0xFF) == 0x10);
...
void test_immediate_alu() {
    ...
    m68k_step(&cpu);
    assert((cpu.d_regs[1].l & 0xFFFF) == 0x20);
    ...
    m68k_step(&cpu);
    assert((cpu.d_regs[0].l & 0xFF) == 0x0F);
    ...
    m68k_step(&cpu);
    assert((cpu.d_regs[0].l & 0xFF) == 0x81);
