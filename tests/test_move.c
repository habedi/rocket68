...
void test_move() {
    ...
    assert(cpu.d_regs[0].l == 0x12345678);
    ...
    assert((cpu.d_regs[1].l & 0xFFFF) == 0x5678);
...
void test_movem() {
    ...
    assert(cpu.d_regs[1].l == 2);
    assert(cpu.a_regs[0].l == 3);
    assert(cpu.a_regs[7].l == 100);
...
void test_movep() {
    ...
    m68k_step(&cpu);
    assert((cpu.d_regs[1].l & 0xFFFF) == 0xABCD);
...
void test_exg() {
    ...
    cpu.d_regs[1].l = 0x22222222;
    m68k_write_16(&cpu, 0, 0xC141);
    m68k_step(&cpu);
    assert(cpu.d_regs[0].l == 0x22222222);
    assert(cpu.d_regs[1].l == 0x11111111);

    // 2. EXG A0, A1
    // Op: 1100 000 1 01001 001 (0xC149) -> A0, A1 exchange
    cpu.a_regs[0].l = 0xAAAAAAAA;
    cpu.a_regs[1].l = 0xBBBBBBBB;
    m68k_write_16(&cpu, 2, 0xC149);
    m68k_step(&cpu);
    assert(cpu.a_regs[0].l == 0xBBBBBBBB);
    assert(cpu.a_regs[1].l == 0xAAAAAAAA);

    // 3. EXG D0, A0
    // Op: 1100 000 1 10001 000 (0xC188) -> D0, A0 exchange
    // Wait: Rx is D0 (000). Mode 10001. Ry is A0 (000).
    // Swap D0 <-> A0.
    cpu.d_regs[0].l = 0xDDDDDDDD;
    cpu.a_regs[0].l = 0xAAAAAAAA;  // Changed in prev step, but let's reset or just expect
    ...
    assert(cpu.a_regs[0].l == 0xDDDDDDDD);
...
void test_stack_frame() {
    ...
    cpu.a_regs[6].l = 0xCAFEBABE;

    m68k_step(&cpu);

    assert(cpu.a_regs[6].l == 0x200 - 4);                 // New FP = Old SP - 4 (pushed A6)
    assert(cpu.a_regs[7].l == 0x200 - 4 - 4);             // New SP = FP - 4
    ...
    assert(cpu.a_regs[7].l == 0x200);       // Restored SP
    assert(cpu.a_regs[6].l == 0xCAFEBABE);  // Restored A6
